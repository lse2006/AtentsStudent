...
namespace _01_Console
{
    public class Orc : Character  
    {
        ...
        int rageMax = 100;  //최대 분노. 상수로 설정. 시작부터 끝까지 변하지 않는다.
        ...
        /// 버서커 모드 설정
        /// </summary>
        /// <param name="on">true면 버서커 모드, false 일반모드</param>
        void BerserkerMode(bool on)
        {
            berserker = on; //변수로 세팅해서 표시
            if (berserker)
            {
            ...
            {
                //strenth /= 2;
                strenth = strenth >> 1; // 일반 모드로 돌아올 때 힘을 절반으로 줄인다.
            }
        }
    }
}